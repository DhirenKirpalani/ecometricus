...
import { Info, Droplets, TrendingDown, X as XIcon } from 'lucide-react';
...
const DEFAULT_COLORS = ['#60A5FA', '#3B82F6', '#d4af37', '#77B139', '#F97316', '#A855F7'];
...
const WaterUsageTemplateChart: React.FC<WaterUsageTemplateChartProps> = ({
    data,
    benchmark,
    outletKeys = [],
    outletColors = {},
        ...
        <div className="bg-[#1c3933] border border-brand-gold/20 rounded-2xl p-5 sm:p-6 shadow-xl w-full h-full flex flex-col transition-all duration-300 hover:border-blue-400/30">
            ...
            <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-base font-geometric font-bold text-white uppercase tracking-tight leading-none">{t('charts.waterTitle')}</h3>
                        <p className="text-[10px] font-bold text-white/40 uppercase tracking-wider mt-1">{t('charts.waterSubtitle')}</p>
                    </div>
                </div>
                {hasAlert ? (
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
                <div className="bg-brand-dark/40 rounded-lg px-3 py-2 border border-brand-gold/5">
                    ...
                    <p className="text-sm font-geometric font-black text-white leading-none mt-1">{Math.round(benchmark * 7).toLocaleString()}<span className="text-[10px] text-white/40 ml-0.5">L/wk</span></p>
                </div>
                <div className="bg-brand-dark/40 rounded-lg px-3 py-2 border border-brand-gold/5">
                    <p className="text-[8px] font-black text-brand-gold/60 uppercase tracking-widest">{t('charts.statWeekly')}</p>
                    <p className="text-sm font-geometric font-black text-white leading-none mt-1">{weeklyTotal.toLocaleString()}<span className="text-[10px] text-white/40 ml-0.5">L</span></p>
                </div>
                <div className="bg-brand-dark/40 rounded-lg px-3 py-2 border border-brand-gold/5">
                    <p className="text-[8px] font-black text-brand-gold/60 uppercase tracking-widest">{t('charts.statAvgDay')}</p>
                    <p className="text-sm font-geometric font-black text-white leading-none mt-1">{Math.round(avgDay).toLocaleString()}<span className="text-[10px] text-white/40 ml-0.5">L</span></p>
                </div>
            </div>
            ...
            <div className="flex-1 w-full relative min-h-0 pb-6">
                ...
                <div className="absolute left-14 right-0 top-0 bottom-6">
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                        <div className="absolute right-0 -translate-y-1/2 flex items-center gap-1" style={{ top: `${getY(benchmark)}%` }}>
                            ...
                            <div className="bg-brand-gold/20 border border-brand-gold/40 px-1.5 py-0.5 rounded text-[7px] font-black text-brand-gold uppercase tracking-wider">
                                {Math.round(benchmark).toLocaleString()}L/d
                            </div>
                        </div>
                    </div>
                    ...
                    <svg className="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 100">
                        <defs>
                            {outletMeta.map(o => (
                                <linearGradient key={o.key} id={`wu-${o.key}`} x1="0" y1="0" x2="0" y2="1">
                    ...
                    {normalizedData.map((d, i) => {
                        ...
                        return (
                            <div key={`dot-${i}`} className="absolute z-20" style={{ left: `${xPct}%`, top: `${yPct}%`, transform: 'translate(-50%, -50%)' }}>
                                <div
                                    className={`w-2.5 h-2.5 rounded-full border-2 cursor-pointer transition-all ${isGood ? 'bg-[#60A5FA] border-[#60A5FA]' : 'bg-brand-alert border-brand-alert'} ${isHovered ? 'scale-150 shadow-lg' : 'hover:scale-125'}`}
                                    onMouseEnter={() => setHoveredDay(i)}
                                    onMouseLeave={() => setHoveredDay(null)}
                                    onClick={() => setSelectedDay(d)}
                                />
                                {isHovered && (
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 bg-brand-dark border border-brand-gold/30 rounded-lg px-2 py-1 shadow-xl whitespace-nowrap z-30 pointer-events-none animate-in fade-in zoom-in duration-150">
                                        ...
                                        {outletMeta.map(o => {
                                            const val = Number((d as any)[o.key]) || 0;
                                            if (!val) return null;
                                            return <p key={o.key} className="text-[9px] font-black" style={{ color: o.color }}>{o.label}: {val.toLocaleString()}L</p>;
                                        })}
                                    </div>
                                )}
                            </div>
                            ...
                            <>
                                ...
                                <div className="absolute bg-brand-dark border border-brand-gold/30 rounded-lg px-3 py-2 shadow-2xl z-50 animate-in fade-in zoom-in duration-200 min-w-[100px]"
                                    ...
                                    {outletMeta.map(o => {
                                        ...
                                        if (!val) return null;
                                        return <p key={o.key} className="text-[10px] font-black text-center" style={{ color: o.color }}>{o.label}: {val.toLocaleString()}L</p>;
                                    })}
                                    <p className={`text-[7px] font-black uppercase text-center mt-1 ${total <= benchmark ? 'text-blue-400/70' : 'text-brand-alert/70'}`}>
